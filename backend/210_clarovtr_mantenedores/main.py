...
from shared.aws_cognito import validate_exist_on_cognito,create_user_on_userpool
...
def return_entidad_post(data, fields, entity):
    ...
    if entity == "tipificacion":
        ...
        data_insert = (
            ...
            timestamp,
            timestamp)
        data_result = insert_data(data_insert, entity, fields)
        if data_result == 'Insertado Correctamente':
            print(data['email'])
            result_cognito = validate_exist_on_cognito(data['email'],secret_value)
            if not result_cognito:
                try:
                    create_user_on_userpool(data['email'], data['password'], data['nombre'] , data['apellidos'], secret_value)
                except Exception as e:
                    print("No se ha podido crear el usuario de migración en el grupo de usuarios: " + data['email'])
                    print(str(e))
...
def return_update_data(data, id_value, entity, fields):
    ...
    if entity == "tipificacion":
        ...
        data = {
            ...
            "email": data['email'],
            "password": encript_password(data['password']),
            "activo": data['activo'],
            "updated_at": timestamp,
            "deleted_at": None,
            "id": data['id']
