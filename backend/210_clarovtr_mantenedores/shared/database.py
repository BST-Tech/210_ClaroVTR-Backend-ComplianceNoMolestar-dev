...
def insert_data(data, entidad, fields):
	...
	if entidad == 'usuario':
		print("usuarios")
		query = f"SELECT * from usuario u WHERE u.email = '{data[2]}'"
		try:
			db = DatabaseConnection()
			if db.connect():
				results = db.execute_query(query)
				print(f"Resultado {results}")
				print(f"largo resultado {len(results)}")
				if len(results) > 0:
					return f"Error al insertar registro {data[2]}, este ya existe"
		except Exception as e:
			print(f"Error general: {e}")
		finally:
			db.close_connection()
...
def soft_delete(id, entidad, fields, timestamp):
	...
	if entidad == "regla":
		...
		value = ""
	elif entidad == "tipificacion":
		print("entidad tipificacion")
		value = ", activo = 0"
	else:
		value = ", activo = 0"
