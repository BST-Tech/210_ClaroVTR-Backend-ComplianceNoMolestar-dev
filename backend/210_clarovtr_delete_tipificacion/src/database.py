...
def delete_data_tipificaciones(id, id_empresa_ct):
    timestamp = datetime.datetime.fromtimestamp(
        time.time()).strftime('%Y-%m-%d %H:%M:%S')
    query = f"UPDATE tipificacion SET activo = 0, deleted_at = '{timestamp}', updated_at = '{timestamp}' WHERE id = {id} and id_empresa_ct = {id_empresa_ct};"
    try:
        db = DatabaseConnection()
        if db.connect():
